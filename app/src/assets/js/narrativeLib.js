...
d3.layout.narrative = function() {
  ...
  //
  // Lightly modified by Matthieu Berthomé Dec 2017
  //
  ...
  function computeScenePositions() {
    scenes.forEach(function(scene) {
      ...
      if (orientation === "vertical") {
        ...
        scene.x =
          scene._x ||
          Math.max(0, Math.min(size[0], scale * scene.start + labelSize[0]));
        scene.y =
          scene._y || Math.max(0, Math.min(size[1], avg - scene.height / 2));
      }
    });
  }
