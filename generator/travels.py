...
def treat_one(character, book_chapter):
    ...
    if (character.is_bob and character == book_chapter.bob_character) or (
        character in book_chapter.characters
    ):
        current_location = book_chapter.location

    character_travel = CharacterTravel(
        character=character, chapter=book_chapter, location=current_location
    )
...
def postprocess_births_deaths(thresholds_last=None, thresholds_first=None):
    ...
    for character_travel in characters_travels:
        ...
        if (threshold_first is not None and (nb, nc) < threshold_first.k) or (
            threshold_last is not None and (nb, nc) > threshold_last.k
        ):
...
def postprocess_character_travels():
    ...
    for character in db.session.query(Character).all():
        current_location_id = None
        for characters_travel in (
            db.session.query(CharacterTravel)
            .filter(CharacterTravel.character == character)
            .order_by(
                CharacterTravel.chapter_nb.asc(), CharacterTravel.chapter_nc.asc()
            )
            .all()
        ):
            if characters_travel.location_id is not None:
    ...
    with contextlib.ExitStack() as stack:
        ...
        for character_id in characters_map.keys():
            ...
            locations_files[character_id] = stack.enter_context(
                open(
                    os.path.join("generated", character_id, "locations"),
                    "w",
                    encoding="utf-8",
                )
            )
        ...
        for character_travel in db.session.query(CharacterTravel).all():
            ...
            try:
                locations_file.write(
                    "{:^10s} {:3d} {:3d} {:s}\n".format(
                        character_id, nb, nc, location_id
                    )
                )
            except TypeError:
                locations_file.write(
                    "{:^10s} {:3d} {:3d} {:s} -> {:s}\n".format(
                        character_id, nb, nc, location_id[0], location_id[1]
                    )
                )
